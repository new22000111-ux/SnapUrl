            ...
            protected String doInBackground(Void... v) {
                try {
                    ...
                    if (responseCode == HttpURLConnection.HTTP_OK) {
                        ...
                        // Use try-with-resources to safely consume and close InputStream.
                        // Removing conn.disconnect() allows the underlying socket to be
                        // reused via HttpURLConnection connection pooling, significantly
                        // improving performance for subsequent network calls.
                        try (BufferedReader reader = new BufferedReader(
                                new InputStreamReader(conn.getInputStream()))) {
                            return reader.readLine();
                        }
                    } else {
                        // Safely consume and close ErrorStream to return connection to pool
